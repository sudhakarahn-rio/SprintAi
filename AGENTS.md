...
- **Cloudflare D1** - SQLite database for data persistence
  ...
  - Database ID: `a80bab21-7502-44a2-a7f1-fc2f599930f0`
  - Binding: `quizmaker_app_database`
  - **Local dev / build**: D1 is simulated locally (Wrangler state under `.wrangler/state`). `next.config.ts` calls `initOpenNextCloudflareForDev({ remoteBindings: false })` so `next dev` and `next build` do not require a Cloudflare remote proxy or workers.dev onboarding. Deployed Workers still use the real D1 database above.
